...
// QUICListener is a stub for the QUIC transport listener.
// A production implementation would use quic-go or similar.
type QUICListener struct {
	config config.ServerConfig
}

func NewQUICListener(cfg config.ServerConfig) *QUICListener {
	return &QUICListener{config: cfg}
}

func (l *QUICListener) Listen(addr string) error { return nil }
func (l *QUICListener) Close() error             { return nil }
...
// TLSConfig builds a *tls.Config for either a client or server.
//
// Fix #9: previous SPKI verification logic used != 0 (meaning: reject when
// equal), which is inverted — a matching SPKI should be accepted, not rejected.
// The correct check is == 0 (not equal → reject).
func TLSConfig(cfg config.ClientConfig, isServer bool) (*tls.Config, error) {
